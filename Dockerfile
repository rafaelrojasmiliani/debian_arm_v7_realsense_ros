...
RUN mkdir  /catkinws/src && \
    ...
    rm -rf /var/lib/apt/lists/* && \
    src/catkin/bin/catkin_make_isolated --install -DCMAKE_BUILD_TYPE=Release -DCATKIN_SKIP_TESTING=ON --install-space /opt/ros/noetic -j3 -DPYTHON_EXECUTABLE=/usr/bin/python3 && \
    cd / && rm -rf /catkinws/*
